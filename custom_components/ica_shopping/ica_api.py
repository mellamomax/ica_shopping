...
class ICAApi:
    ...
    async def async_initialize(self):
        """Läs cookies från lagring eller använd hårdkodade första gången."""
    ...
    async def _async_store_cookies(self):
        """Använd hårdkodade cookies tillfälligt för test."""
    ...
    def _ensure_token(self):
        """Hämta nytt access-token från ICA med cookies."""
        ...
        _LOGGER.debug("Got new ICA token: %s, expires at %s", self._token, self._expires)

    def get_headers(self) -> dict:
        """Returnera giltiga headers med token."""
        self._ensure_token()
        return {
